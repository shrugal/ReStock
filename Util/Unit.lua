...
function Self.ColoredName(name, unit, class)
    return ("|c%s%s|r"):format(Self.Color(unit or name, class).colorStr, name)
end

-- It's just such a common usecase
function Self.ColoredShortenedName(unit, class)
    return unit and Self.ColoredName(Self.ShortenedName(unit), unit, class)
end
...
function Self.Color(unit, class)
    return RAID_CLASS_COLORS[class or select(2, UnitClass(Self(unit))) or "PRIEST"]
end

...
LibStub("LibRevision"):Set("$URL: http://svn.norganna.org/libs/trunk/LibExtraTip/LibExtraTip.lua $","$Rev: 435 $","5.15.DEV.", 'auctioneer', 'libs')
	...
	if lib.sortedCallbacks and #lib.sortedCallbacks > 0 then
		for i,options in ipairs(lib.sortedCallbacks) do
			if options.type == tiptype then
				...
				if enable then
					-- if tiptype == "item" then print(...) end
